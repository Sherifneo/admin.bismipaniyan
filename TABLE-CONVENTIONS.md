...
Fully applied (Dynamics-style column headers + SearchByBar + operator-driven filters): Retail Stores, Customers, Products, Vendors, Machines, Partners (+ its Settlements sub-table), Sales Orders, Purchase Orders, Cash Book (+ its Ledger Transaction tab), Inventory, WhatsApp Orders, Cash & Bank (renamed from "Bank Accounts" — its Accounts tab and its Bank Transaction tab, `src/pages/bank-accounts/BankAccountsList.jsx`), Production Runs, Cost Parameters, Team, Reports (all three tabs), Activity Log.
